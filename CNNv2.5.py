...
train_Img, train_Lab, validation_Img, validation_Lab, test_Img, test_Lab = DataManager.GetMarcinDatasetAndOurRotated()
...
modelName = "CNNv25_OUHRotatedAndMarcin_DropOut04"
filepath = "checkpoints/chpt_CNNv25_OUHRotatedAndMarcin_DropOut06-{epoch:04d}-loss-{val_loss:.2f}-Metric-{val_mean_absolute_error:.3f}.h5"
...
model.fit(x=np.array(train_Img), y=np.array(train_Lab), batch_size=16, epochs=100, validation_data=(np.array(validation_Img) , np.array(validation_Lab) ),
          callbacks=[logger])
